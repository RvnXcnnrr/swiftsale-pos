...
class MainProductAPIController extends AppBaseController
{
    ...
    public function store(CreateMainProductRequest $request): MainProductResource|JsonResponse
    {
        $input = $request->all();

        \Log::info('MainProductAPIController@store input:', $input);

        if (isset($input['barcode_symbol']) && $input['barcode_symbol'] == Product::EAN8 && strlen($input['code'] ?? '') != 7) {
            return $this->sendError('Please enter 7 digit code', 422);
        }

        if (isset($input['barcode_symbol']) && $input['barcode_symbol'] == Product::UPC && strlen($input['code'] ?? '') != 11) {
            return $this->sendError('Please enter 11 digit code', 422);
        }

        // Check if either gd or imagick extension is loaded
        if (!extension_loaded('gd') && !extension_loaded('imagick')) {
            return $this->sendError('Neither gd-lib or imagick are installed!', 422);
        }
        ...
        try {
            ...
            if (isset($input['images']) && !empty($input['images'])) {
                try {
                    ...
                    \Log::error('Image processing error in MainProductAPIController@store:', ['message' => $e->getMessage()]);
                    // Optionally, you can return an error response or continue without images
                    // return $this->sendError('Image processing failed: ' . $e->getMessage(), 422);
                }
            }
            ...
            DB::rollBack();
            \Log::error('Validation error in MainProductAPIController@store:', $ve->errors());
            return $this->sendError($ve->getMessage(), 422);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Exception in MainProductAPIController@store:', ['message' => $e->getMessage()]);

...
const EditProductScreen = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    logInfo(LOG_CATEGORIES.PRODUCTS, 'editProduct_form_submit', { productId, formData });

    if (!validateForm()) {
      logError(LOG_CATEGORIES.PRODUCTS, 'editProduct_validation_failed', { errors });
    ...
    try {
      ...
      logInfo(LOG_CATEGORIES.PRODUCTS, 'editProduct_submitting', { productData });

      // Dispatch update product action
      const result = await dispatch(updateProduct({ id: productId, productData })).unwrap();

      logInfo(LOG_CATEGORIES.PRODUCTS, 'editProduct_success', { result });
      ...
      logError(LOG_CATEGORIES.PRODUCTS, 'editProduct_failed', { error: error.message });
